...
func TestSessionPromptMetaSkipsBasePromptForRestrictedWorker(t *testing.T) {
	manager := &Manager{cfg: Config{SystemPrompt: testPrompt("jaz platform prompt")}}
	for _, policy := range []string{MCPServerPolicyMemorySearchWorker, MCPServerPolicyMemorySourceWorker} {
		got, err := manager.systemPrompt(context.Background(), "", "", policy, []string{"memory worker prompt"})
		if err != nil {
			t.Fatal(err)
		}
		if got != "memory worker prompt" {
			t.Fatalf("%s system prompt = %#v", policy, got)
		}
	}
}
